...
impl LookupTable {
    ...
    /// Returns `Err` if backends is empty.
    pub fn build(backends: &[Backend], m: usize) -> Result<Self, &'static str> {
        if backends.is_empty() {
            return Err("cannot build lookup table with empty backend list");
        }

        let n = backends.len();
        let mut entries = vec![usize::MAX; m];

        // Pre-compute offset/skip pairs instead of materializing full preference lists.
        // This reduces memory from O(n*m) (~50MB for n=100, m=65537) to O(n) (~1.6KB).
        ...
        'outer: loop {
            for i in 0..n {
                let (o, s) = params[i];
                // Compute preference list entry on-the-fly: (offset + next * skip) % m
